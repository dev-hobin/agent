...
test("live failure classification keeps wrong answers distinct from ordinary variance", () => {
	assert.equal(
		classifyLiveFailure("structurally inadmissible first target"),
		"inadmissible-result",
	);
	assert.equal(
		classifyLiveFailure("wall-clock budget 10/1ms"),
		"budget-exhausted",
	);
	assert.equal(
		classifyLiveFailure("wrong-work-id"),
		"protocol-attempt-rejected",
	);
