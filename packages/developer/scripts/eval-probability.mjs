...
export function classifyLiveFailure(output) {
	if (
		/structurally inadmissible|omitted required semantic term|without an active change authorization|mutation started before change authorization|outcome .* is not allowed/.test(
			output,
		)
	) {
	...
	if (
		/decision budget|tool-call budget|tool-error budget|wall-clock budget|no-progress budget/.test(
			output,
		)
	) {
	...
	if (
		/open judgment failed|developer_conclude_judgment failed|wrong-work-id|unknown pending question/i.test(
			output,
		)
	) {
	...
	if (
		/requires PI_CODING_AGENT_DIR|Unknown DEVELOPER_EVAL_FIXTURE|Pi exited/.test(
			output,
		)
	) {
...
export function parseEvalResult(output) {
	...
	try {
		...
		throw new Error(
			`Invalid live eval result: ${error instanceof Error ? error.message : String(error)}`,
			{
				cause: error,
			},
		);
...
export function summarizeTrialObservations(fixtureId, observations) {
	...
	const preferred = observations.filter(
		(observation) => observation.classification === "preferred",
	).length;
	const inadmissible = observations.filter(
		(observation) => observation.classification === "inadmissible-result",
	).length;
	return {
		fixtureId,
		trials,
