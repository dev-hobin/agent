...
import {
	...
	snapshotWorkspace,
} from "../scripts/eval-filesystem.mjs";
...
test("the eval observer rejects artifact changes without authorization and permits authorized mutation", async () => {
	...
	try {
		...
		assert.match(
			violation?.content.at(-1)?.text ?? "",
			/without an active change authorization/,
		);

		await toolResult({
			toolName: "developer_authorize_change",
			toolCallId: "authorize:implementation",
			isError: false,
			content: [],
		});
		...
		if (previousWorkspace === undefined)
			delete process.env.DEVELOPER_EVAL_WORKSPACE;
