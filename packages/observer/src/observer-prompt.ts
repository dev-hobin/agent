...
function renderMemoRequestContext(input: {
	readonly requestId: string;
	readonly observationIds: readonly string[];
	readonly finalReviewReconciliation: boolean;
	readonly piggyback: boolean;
	readonly lockedContext?: string | null;
}): string {
	const lines = [
		"<observer-memo-request>",
		`request_id=${input.requestId}`,
		`observation_ids=${input.observationIds.join(",")}`,
	];
	if (input.finalReviewReconciliation) {
		lines.push(
	...
	if (input.piggyback && input.lockedContext) {
		lines.push(
			"The extension loaded Memo context locally from the current branch. Do not call load-memo-context.",
			"Set observer-commit.memo to this exact request_id and the submission described by the locked context below; do not call load-memo-context or reconcile-memo separately.",
			`locked_memo_context=${input.lockedContext}`,
		);
	} else {
		lines.push(
			"Call observer_sidecar action load-memo-context with this exact request ID.",
			"The load-memo-context result includes producer-owned locked fields and memo_reconciliation.submission_seed.",
			"Call reconcile-memo with the same request ID and one submission containing only evidence, hypothesis_outcomes, memo_outcomes, and dispositions; never resend or nest locked fields.",
		);
	}
	lines.push(
...
function renderSaveRequestContext(input: {
	readonly requestId: string;
	readonly piggyback: boolean;
	readonly lockedContext?: string | null;
}): string {
	const lines = ["<observer-save-request>", `request_id=${input.requestId}`];
	if (input.piggyback && input.lockedContext) {
		lines.push(
			"The extension loaded save context locally from the current branch. Do not call load-save-context.",
			"Set observer-commit.save to only request_id, summary, and records from the locked context below; do not call load-save-context or prepare-save-proposal separately.",
			`locked_save_context=${input.lockedContext}`,
		);
	} else {
		lines.push(
			"Call observer_sidecar action load-save-context with this exact request ID exactly once unless it returns an error.",
			"The successful load-save-context result is read-only and returns next_action=prepare-save-proposal plus required records and the producer-owned locked target.",
			"After loading context successfully, do not call load-save-context again. Follow next_action and submit only request_id, summary, and records.",
		);
	}
	lines.push(
		"Do not invent or resend notebook root, notebook ID, episode language, proposal ID, or request digest.",
		"Only prepare-save-proposal completion may claim that Review produced a proposal. It must not claim approval, file writes, or Episode settlement.",
		"</observer-save-request>",
	);
	return lines.join("\n");
}

function renderHypothesisReviewContext(
	hypotheses: readonly {
...
type ObservationSession = ReturnType<typeof reconstructObservationSession>;

function renderPiggybackPolicy(input: {
	...
	if (input.standingIndex) {
		lines.push(`current_standing_index=${input.standingIndex}`);
	}
	lines.push(
		"Each observations item combines existing candidate_ids and/or current-run nominations with source, faithful_summary, claims, related_inquiry_ids, stance, and one record choice. The extension performs record-source-reading, optional load-inquiry-context, and record-observation behind one validated boundary.",
		"That commit terminates this agent run without a follow-up model request. Do not retry a rejected commit in the same run.",
		"Memo and Save cannot share one commit because Save context depends on completed Memo. Leave the later Save stage pending for a later ordinary user turn.",
		"</observer-piggyback-policy>",
	);
	return lines.join("\n");
}
...
function renderCandidateLine(
	candidate: ObservationSession["candidates"][number],
): string {
	let selection = "";
	if (
		...
		selection = ` selected=${JSON.stringify(excerpt(candidate.origin.nominationReason))}`;
	}
	return `- ${candidate.candidateId} [${candidate.origin.kind}]${selection} ${JSON.stringify(excerpt(candidate.text))}`;
}

function renderNominatableLine(result: ObserverNominatableToolResult): string {
	const status = result.isError ? "error" : "success";
	return `- ${result.toolCallId} tool=${JSON.stringify(result.toolName)} status=${status}`;
}

function routineSteps(piggyback: boolean): readonly string[] {
	if (piggyback) {
		return [
	...
	return [
		...
		"4. Call record-source-reading with the candidate IDs returned by nomination or listed below and faithful Source facts/claims; it then returns only the compact StandingIndex.",
		"5. If related IDs are plausible, call load-inquiry-context for only those IDs; it returns selected full context.",
		"6. Call record-observation with observer_hypothesis=null for an ordinary observation. Call record-new-hypothesis only for an independent new Observer hypothesis. Use user-hypothesis for an explicit user proposal.",
...
function renderRoutineContext(input: {
	readonly session: ObservationSession;
	readonly nominatableToolResults: readonly ObserverNominatableToolResult[];
	readonly requestContexts: readonly string[];
	readonly piggybackGuidance: string | null;
	readonly piggyback: boolean;
}): string {
	const usedCandidateIds = new Set(
		input.session.sourceReads.flatMap((read) => read.candidateIds),
	);
	for (const hypothesis of input.session.userHypotheses) {
		usedCandidateIds.add(hypothesis.candidateId);
	}
	const pendingCandidates = input.session.candidates.filter(
		(candidate) =>
			!candidate.materialReviewRequestId &&
			!usedCandidateIds.has(candidate.candidateId),
	);
	const observedReadIds = new Set(
		input.session.observations.map((observation) => observation.readId),
	);
	const pendingReads = input.session.sourceReads.filter(
		(read) =>
	...
	const nominatableLines = input.nominatableToolResults.map(
		renderNominatableLine,
	);
	const readLines = pendingReads.map((read) => {
		const hydration = input.session.hydrations.find(
			(item) => item.readId === read.readId,
		);
		if (hydration) {
			return `- ${read.readId}: hydrated=${hydration.hydrationId} inquiries=${hydration.inquiryIds.join(",")}`;
		}
		return `- ${read.readId}: source-read complete; StandingIndex digest=${read.indexDigest}`;
	});
	const lines: string[] = [];
	if (input.piggybackGuidance) {
		lines.push(input.piggybackGuidance, "");
	}
	if (input.requestContexts.length > 0) {
		lines.push(input.requestContexts.join("\n\n"), "");
	}
	lines.push(
		"<observer-sidecar>",
		...
		...routineSteps(input.piggyback),
		"Minor/support/uncertain observations stay quiet. The tool itself alerts only after a Major event is appended.",
		"",
		`Tool results eligible for meaning-based nomination in this agent run (${input.nominatableToolResults.length}):`,
	...
	if (pendingCandidates.length > MAX_CONTEXT_CANDIDATES) {
		lines.push(`- … ${pendingCandidates.length - MAX_CONTEXT_CANDIDATES} more`);
	}
	lines.push(
		"",
...
export function observerSidecarContext(
	...
	const memoContext = pendingMemo
		? renderMemoRequestContext({
				requestId: pendingMemo.requestId,
				observationIds: pendingMemo.observationIds,
				finalReviewReconciliation: reviewSaveMemoRequestIds.has(
					pendingMemo.requestId,
				),
				piggyback: Boolean(options.piggyback),
				lockedContext: options.memoScope,
			})
		: null;
	const pendingSave = saveSession.pendingRequest;
	const saveContext = pendingSave
		? renderSaveRequestContext({
				requestId: pendingSave.requestId,
				piggyback: Boolean(options.piggyback),
				lockedContext: options.saveScope,
			})
		: null;
	const hypothesisReviewContext = renderHypothesisReviewContext(
		session.pendingHypothesisReviews,
		Boolean(options.piggyback),
	);
	...
	const piggybackGuidance = options.piggyback
		? renderPiggybackPolicy({
				episodeId: session.lifecycle.episode.core.episodeId,
				standingIndex: options.standingIndex,
			})
		: null;
