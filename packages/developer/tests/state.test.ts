...
const enabledState = () =>
	applyDeveloperEvent(initialState(), {
		protocol: PROTOCOL,
		kind: "activation",
		enabled: true,
	});

const reconstructEnabledState = (
	entries: Parameters<typeof reconstructState>[0],
) => reconstructState([activationEntry, ...entries]);
...
test("turning Developer off clears optional route, focus, question, and debt state", () => {
	const dirtyState: DeveloperState = {
		...enabledState(),
		activeRoute: route,
		lastRoute: route,
		routeHistory: [route],
		pendingQuestions: [
			{
				...
				resolutionCriteria: "Observe the remaining behavior.",
				sourceRouteId: route.routeId,
			},
		],
		focusedQuestionId: "question:dirty",
	...
	assert.deepEqual(
		applyDeveloperEvent(dirtyState, {
			protocol: PROTOCOL,
			kind: "activation",
			enabled: false,
		}),
...
test("reconstructs activation and protocol state from the active branch", () => {
	const entries = [
		{
			type: "custom",
			customType: ACTIVATION_ENTRY,
...
test("reconstructs optional pending-question context", () => {
	...
	const state = reconstructEnabledState([
		toolEntry(ROUTE_TOOL, route),
		toolEntry(JUDGMENT_TOOL, openQuestionJudgment),
	]);

	assert.equal(
		state.pendingQuestions[0]?.context,
		"Choose one:\n- absent\n- explicitly cleared",
	);
...
test("branch reconstruction ignores events from another branch", () => {
	...
	const otherRoute = {
		...
		target: "naming-judgment",
	};
	const secondBranch = reconstructState([
		base,
		toolEntry(ROUTE_TOOL, otherRoute),
	]);
		...
		openedQuestions: [
			{
				...
				gate: "none",
				resolutionCriteria:
					"Identify the concrete missing environment observation.",
				sourceRouteId: route.routeId,
			},
		],
	});
		...
		openedQuestions: [
			{
				...
				gate: "before-completion",
				resolutionCriteria:
					"Observe a successful checkout interaction at 320px.",
...
test("a user-owned before-implementation question is visibly blocking", () => {
	let state = applyDeveloperEvent(enabledState(), route);
	state = applyDeveloperEvent(state, {
		...resolved(route),
		status: "blocked",
...
test("an unrelated resolved route cannot hide an existing blocker", () => {
	...
	state = applyDeveloperEvent(
		state,
		resolved(unrelated, "The name preserves domain meaning."),
	);
...
test("resolved model work requires sketch or signal framing before implementation", () => {
	const modelRoute = { ...route, target: "model" };
	let state = applyDeveloperEvent(enabledState(), modelRoute);
	state = applyDeveloperEvent(state, resolved(modelRoute));
	assert.equal(state.implementationFramingRequired, true);

	const sketchRoute = { ...route, routeId: "route:sketch", target: "sketch" };
...
test("a changed implementation landing requires a later resolved verify judgment", () => {
	const implementationRoute = { ...route, target: "implementation" };
	let state = applyDeveloperEvent(enabledState(), implementationRoute);
	state = applyDeveloperEvent(state, {
		...resolved(implementationRoute),
	...
	const verifyRoute = { ...route, routeId: "route:verify", target: "verify" };
...
test("before-completion questions keep verification debt until their criteria are resolved", () => {
	...
	let state = applyDeveloperEvent(enabledState(), questionRoute);
	state = applyDeveloperEvent(state, {
		...
		openedQuestions: [
			{
				...
				gate: "before-completion",
				resolutionCriteria:
					"The user explicitly accepts the rendered behavior.",
...
test("stale judgments cannot close a different active route", () => {
	const state = applyDeveloperEvent(
		applyDeveloperEvent(enabledState(), route),
		{
			...resolved(route),
			routeId: "route:stale",
		},
...
test("a second route event cannot overwrite an active route during replay", () => {
	const first = applyDeveloperEvent(enabledState(), route);
...
test("ignores pre-v5 session entries", () => {
	const previousProtocol = "developer/v4";
	const state = reconstructState([
		{
			type: "custom",
			customType: ACTIVATION_ENTRY,
			data: { protocol: previousProtocol, kind: "activation", enabled: true },
		},
		toolEntry(ROUTE_TOOL, { ...route, protocol: previousProtocol }),
	...
	assert.equal(state.activeRoute, undefined);
	assert.equal(state.lastJudgment, undefined);
	assert.deepEqual(state.routeHistory, []);
	assert.deepEqual(state.judgmentHistory, []);
});
...
test("replays valid implementation profiles and rejects malformed ones", () => {
	const implementationRoute: RouteEvent = {
		...route,
		target: "implementation",
		executionProfile: "behavior-preserving-structure",
	};
	const valid = reconstructEnabledState([
		toolEntry(ROUTE_TOOL, implementationRoute),
	]);
	assert.equal(
		valid.activeRoute?.executionProfile,
		"behavior-preserving-structure",
	);

	const invalid = reconstructEnabledState([
		toolEntry(ROUTE_TOOL, {
			...implementationRoute,
