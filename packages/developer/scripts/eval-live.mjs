...
const trials = positiveInteger(process.env.DEVELOPER_EVAL_TRIALS || "3", "DEVELOPER_EVAL_TRIALS");
const runner = process.env.DEVELOPER_EVAL_RUNNER ||
  join(root, "scripts", transport === "rpc" ? "eval-rpc.mjs" : "eval-json.mjs");
const trialTimeoutMs = Number(process.env.DEVELOPER_EVAL_TRIAL_TIMEOUT_MS || 300000);
const report = {
...
for (const fixture of fixtures) {
  ...
  for (let trial = 1; trial <= trials; trial += 1) {
    ...
    if (result?.structuralValid && fixture.admissibleFirstOwners.includes(result.firstOwner)) {
      const preferredFirstOwner = fixture.preferredFirstOwners.includes(result.firstOwner);
      observations.push({
        trial,
        ...result,
        preferredFirstOwner,
        classification: preferredFirstOwner ? "preferred" : "admissible-non-preferred",
      });
      ...
      observations.push({
        ...
        evidence: `Runner accepted inadmissible first owner ${result.firstOwner}`,
      });
    } else {
      observations.push({
        trial,
...
const reportPath = process.env.DEVELOPER_EVAL_REPORT ||
  join(tmpdir(), `developer-probabilistic-eval-${transport}-${Date.now()}.json`);
...
if (report.fixtures.some((fixture) => fixture.inadmissible > 0)) process.exitCode = 1;

const minimum = process.env.DEVELOPER_EVAL_MIN_ACCEPTANCE_RATE;
if (minimum !== undefined) {
  const threshold = Number(minimum);
  if (!Number.isFinite(threshold) || threshold < 0 || threshold > 1) {
    throw new Error("DEVELOPER_EVAL_MIN_ACCEPTANCE_RATE must be between 0 and 1");
  }
  if (report.fixtures.some((fixture) => fixture.acceptanceRate < threshold)) process.exitCode = 1;
}
