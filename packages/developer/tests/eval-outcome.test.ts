...
test("eval outcomes distinguish unchanged, pending, paused, and verified paths", () => {
  assert.equal(classifyEvalOutcome({ changes: [], status: status() }), "settled-unchanged");
  assert.equal(
    classifyEvalOutcome({ changes: [], status: status({ protocol: "needs-answer", pending: "question:1" }) }),
    "pending",
  );
  assert.equal(
    classifyEvalOutcome({
      changes: [{ path: "src/file.ts", kind: "modified" }],
      status: status({ protocol: "needs-routing", checkpoint: "reroute required", verification: "required" }),
    }),
    "changed-paused",
  );
  assert.equal(
    classifyEvalOutcome({ changes: [{ path: "src/file.ts", kind: "modified" }], status: status() }),
    "changed-verified",
  );
});
...
test("JSON event replay recovers checkpoint and verification state without TUI status events", () => {
  ...
  const events = [route, judgment].map((details, index) => ({
    ...
    toolName: index === 0 ? "developer_route_question" : "developer_record_judgment",
    isError: false,
    result: { details },
  }));

  assert.deepEqual(statusFromDeveloperEvents(events, "strict"), {
    protocol: "needs-routing",
    active: "none",
    checkpoint: "reroute required",
    verification: "required",
...
test("fixture outcome declarations reject pass-but-wrong terminal states", () => {
  assert.doesNotThrow(() =>
    assertAllowedOutcome({ id: "change", allowedOutcomes: ["changed-verified"] }, "changed-verified"),
  );
  assert.throws(
    () => assertAllowedOutcome({ id: "change", allowedOutcomes: ["changed-verified"] }, "changed-paused"),
    /not allowed/,
  );
});
