...
const configDir =
	...
	(await mkdtemp(join(tmpdir(), "learning-pi-rpc-")));
...
try {
	...
	const learningCommand = response.data.commands.find(
		(entry) => entry.name === "learning",
	);
	assert.equal(learningCommand?.source, "extension");
	assert.match(learningCommand?.description ?? "", /Learning approach/);
	const skillCommands = response.data.commands.filter(
		(entry) => entry.source === "skill",
	);
	assert.deepEqual(skillCommands.map((entry) => entry.name).sort(), [
		"skill:conceptualize",
		"skill:exercise",
	...
	for (const command of skillCommands) {
		...
		assert.ok(
			String(command.sourceInfo.path).startsWith(
	...
	assert.doesNotMatch(stderr, /failed to load|unknown command/i);
	process.stdout.write(
		`Pi RPC smoke: learning package loaded its chooser and five independent skill commands on Pi ${piVersion}\n`,
	);
} finally {
	child.kill("SIGTERM");
}
