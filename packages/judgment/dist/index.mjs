...
function CollapseIntersectProperties(left, right) {
  ...
  const unique3 = memory_exports.Assign(leftProperties, rightProperties);
  const shared = memory_exports.Assign(unique3, sharedProperties);
  return shared;
}
...
// src/errors.ts
var JudgmentError = class extends Error {
  code;
  details;
...
var JudgmentParseError = class extends JudgmentError {
  constructor(message, details = {}, options) {
    super("JUDGMENT_PARSE_INVALID", message, details, options);
  }
};
var ContextSealError = class extends JudgmentError {
  constructor(message, details = {}, options) {
    super("JUDGMENT_CONTEXT_SEAL_FAILED", message, details, options);
  }
};
var JudgmentTransitionError = class extends JudgmentError {
  constructor(message, details = {}, options) {
...
var JsonValueError = class extends Error {
  path;
...
function jsonValueFromUnknown(value, path = "/") {
  if (value === null || typeof value === "string" || typeof value === "boolean")
    return value;
  if (typeof value === "number" && Number.isFinite(value)) return value;
  if (Array.isArray(value)) {
    return value.map(
      (entry, index) => jsonValueFromUnknown(entry, `${path}${index}/`)
    );
  }
...
function decodeJsonValue(source) {
  let parsed;
  try {
...
var NonEmptyTextSchema = typebox_exports.String({
  minLength: 1,
  maxLength: 4e3,
  pattern: "\\S"
});
var IdentifierSchema = typebox_exports.String({
  minLength: 1,
  maxLength: 160,
  pattern: "^[A-Za-z][A-Za-z0-9._:/-]*$"
});
var Sha256Schema = typebox_exports.String({ pattern: "^[a-f0-9]{64}$" });
var ContextUseSchema = typebox_exports.Union([
  typebox_exports.Literal("constraint"),
  typebox_exports.Literal("evidence"),
  typebox_exports.Literal("decision"),
  typebox_exports.Literal("method"),
  typebox_exports.Literal("guidance")
]);
var AssuranceSchema = typebox_exports.Union([
  typebox_exports.Literal("agent-asserted"),
  typebox_exports.Literal("domain-verified"),
  typebox_exports.Literal("user-accepted")
]);
var StatementsSchema = typebox_exports.Array(
  typebox_exports.String({ minLength: 1, maxLength: 2e3, pattern: "\\S" }),
  { minItems: 1, maxItems: 64, uniqueItems: true }
);
var PreparedReferencePolicyDataSchema = exactObject({
  path: typebox_exports.String({ minLength: 1, maxLength: 1024, pattern: "\\S" }),
  when: StatementsSchema
});
var JudgmentAuthoringPolicyDataSchema = exactObject({
  $schema: typebox_exports.Optional(typebox_exports.String({ minLength: 1, maxLength: 2e3 })),
  specVersion: typebox_exports.Literal("0.1"),
  when: StatementsSchema,
  unless: StatementsSchema,
  references: typebox_exports.Array(PreparedReferencePolicyDataSchema, {
    minItems: 1,
    maxItems: 128
  })
});
...
// src/authoring.ts
var authoringPolicyBrand = Symbol("JudgmentAuthoringPolicy");
function schemaIssues(value) {
  return [...Errors2(JudgmentAuthoringPolicyDataSchema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
}
function decodeJudgmentAuthoringPolicyData(value) {
  try {
...
function statements(values, path) {
...
function parsePreparedReferencePath(value, path) {
  if (value !== value.trim() || value.includes("\\") || value.includes("\0") || posix.isAbsolute(value)) {
    throw new JudgmentParseError(
...
function parseJudgmentAuthoringPolicy(data) {
  const when = statements(data.when, "/when");
  const unless = statements(data.unless, "/unless");
  const references = data.references.map(reference).toSorted((left, right) => left.path.localeCompare(right.path));
...
function parseJudgmentAuthoringPolicyJson(source) {
  try {
...
function canonicalJudgmentAuthoringPolicy(policy) {
...
// src/authoring-json-schema.ts
var JudgmentAuthoringJsonSchema = Object.freeze({
...
// src/compiled-policy.ts
function exactObject2(properties) {
  return typebox_exports.Object(properties, { additionalProperties: false });
}
var PolicyOwnerDataSchema = exactObject2({
  kind: typebox_exports.Union([
    typebox_exports.Literal("pi-skill"),
    typebox_exports.Literal("adapter-capability")
  ]),
  namespace: typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
  name: typebox_exports.String({
    minLength: 1,
    maxLength: 160,
    pattern: "^[A-Za-z][A-Za-z0-9._:/-]*$"
  }),
  provenance: exactObject2({
    source: typebox_exports.String({ minLength: 1, maxLength: 1e3, pattern: "\\S" }),
    scope: typebox_exports.Union([
      typebox_exports.Literal("user"),
      typebox_exports.Literal("project"),
      typebox_exports.Literal("temporary")
    ]),
    origin: typebox_exports.Union([
      typebox_exports.Literal("package"),
      typebox_exports.Literal("top-level"),
...
var StatementsDataSchema = typebox_exports.Array(
  typebox_exports.String({ minLength: 1, maxLength: 2e3, pattern: "\\S" }),
  { minItems: 1, maxItems: 64, uniqueItems: true }
);
var CompiledJudgmentPolicyDataSchema = exactObject2({
  specVersion: typebox_exports.Literal("0.1"),
  owner: PolicyOwnerDataSchema,
  when: StatementsDataSchema,
  unless: StatementsDataSchema,
  references: typebox_exports.Array(
    exactObject2({
      path: typebox_exports.String({ minLength: 1, maxLength: 1024, pattern: "\\S" }),
      when: StatementsDataSchema
    }),
  ...
  authoringSha256: typebox_exports.String({ pattern: "^[a-f0-9]{64}$" }),
  policySha256: typebox_exports.String({ pattern: "^[a-f0-9]{64}$" })
});
var policyOwnerBrand = Symbol("PolicyOwner");
var compiledPolicyBrand = Symbol("CompiledJudgmentPolicy");
function issues(value) {
  return [...Errors2(PolicyOwnerDataSchema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
...
function decodePolicyOwnerData(value) {
  try {
...
function parsePolicyOwner(data) {
  const owner = {
    ...
    provenance: Object.freeze({
      ...
      ...data.provenance.path ? { path: nonBlank(data.provenance.path, "/provenance/path") } : {}
    })
  };
  return Object.freeze(owner);
}
function policyOwnerIdentity(owner) {
  return {
    kind: owner.kind,
...
function compiledIdentity(input) {
  return {
    ...
    references: input.policy.references.map((reference2) => ({
...
function compileJudgmentPolicy(input) {
  const policySha256 = sha256(canonicalJson(compiledIdentity(input)));
  const references = Object.freeze(
    input.policy.references.map(
      (reference2) => Object.freeze({
        path: reference2.path,
        when: reference2.when,
        referenceId: reference2.path
      })
    )
  );
  const policy = {
...
function compiledJudgmentPolicyData(policy) {
  return Object.freeze({
    specVersion: "0.1",
    owner: {
      ...
      provenance: {
        ...
        ...policy.owner.provenance.path ? { path: policy.owner.provenance.path } : {}
      }
    },
    when: [...policy.when],
    unless: [...policy.unless],
    references: policy.references.map((reference2) => ({
      path: reference2.path,
      when: [...reference2.when]
    })),
...
function decodeCompiledJudgmentPolicyData(value) {
  try {
    ...
    throw new JudgmentParseError(
      ...
      {
        issues: [...Errors2(CompiledJudgmentPolicyDataSchema, value)].map(
          (issue) => ({
            path: issue.instancePath || "/",
            message: issue.message,
            keyword: issue.keyword
...
function parseCompiledJudgmentPolicy(data) {
...
function canonicalCompiledJudgmentPolicy(policy) {
  return canonicalJson({
...
// src/context.ts
function exactObject3(properties) {
  return typebox_exports.Object(properties, { additionalProperties: false });
}
var ContextProvenanceDataSchema = exactObject3({
  source: typebox_exports.String({ minLength: 1, maxLength: 1e3, pattern: "\\S" }),
  scope: typebox_exports.Union([
    typebox_exports.Literal("user"),
    typebox_exports.Literal("project"),
...
var SourceBase = {
  id: IdentifierSchema,
  title: typebox_exports.String({ minLength: 1, maxLength: 500, pattern: "\\S" }),
  description: typebox_exports.String({ minLength: 1, maxLength: 2e3, pattern: "\\S" }),
  provenance: ContextProvenanceDataSchema
};
var PreparedReferenceDescriptorDataSchema = exactObject3({
  ...SourceBase,
  kind: typebox_exports.Literal("prepared-reference"),
  path: typebox_exports.String({ minLength: 1, maxLength: 1024, pattern: "\\S" }),
  when: typebox_exports.Array(NonEmptyTextSchema, {
    minItems: 1,
    maxItems: 64,
    uniqueItems: true
  }),
...
var PiSkillDescriptorDataSchema = exactObject3({
  ...SourceBase,
...
var PiContextFileDescriptorDataSchema = exactObject3({
  ...SourceBase,
  kind: typebox_exports.Literal("pi-context-file"),
  path: typebox_exports.String({ minLength: 1, maxLength: 2e3, pattern: "\\S" }),
  contentSha256: Sha256Schema
});
var ContextSourceDescriptorDataSchema = typebox_exports.Union([
  PreparedReferenceDescriptorDataSchema,
  PiSkillDescriptorDataSchema,
  PiContextFileDescriptorDataSchema
]);
var CapabilityDescriptorDataSchema = exactObject3({
  id: IdentifierSchema,
  kind: typebox_exports.Literal("pi-tool"),
  name: typebox_exports.String({ minLength: 1, maxLength: 128, pattern: "\\S" }),
  description: typebox_exports.String({ minLength: 1, maxLength: 2e3, pattern: "\\S" }),
  active: typebox_exports.Boolean(),
  provenance: ContextProvenanceDataSchema
});
var ContextInventoryDataSchema = exactObject3({
  sources: typebox_exports.Array(ContextSourceDescriptorDataSchema, { maxItems: 2e3 }),
  capabilities: typebox_exports.Array(CapabilityDescriptorDataSchema, { maxItems: 1e3 })
});
var ObservedBase = {
  id: IdentifierSchema,
  contentSha256: Sha256Schema,
  ...
  sequence: typebox_exports.Integer({ minimum: 0, maximum: 1e9 }),
  provenance: ContextProvenanceDataSchema
};
var ReadResultDataSchema = exactObject3({
  ...ObservedBase,
  kind: typebox_exports.Literal("read-result"),
  toolCallId: typebox_exports.String({ minLength: 1, maxLength: 300 }),
  toolName: typebox_exports.String({ minLength: 1, maxLength: 128 }),
  argumentsSha256: Sha256Schema,
  inventorySourceId: typebox_exports.Optional(IdentifierSchema)
});
var ToolResultDataSchema = exactObject3({
  ...ObservedBase,
...
var UserExplicitDataSchema = exactObject3({
  ...ObservedBase,
  kind: typebox_exports.Literal("user-explicit"),
  userEventId: typebox_exports.String({ minLength: 1, maxLength: 300 })
});
var DomainEvidenceDataSchema = exactObject3({
  ...ObservedBase,
  kind: typebox_exports.Literal("domain-evidence"),
  domainRef: typebox_exports.String({ minLength: 1, maxLength: 300 }),
  domainType: IdentifierSchema,
  evaluator: exactObject3({
    id: IdentifierSchema,
    version: typebox_exports.String({ minLength: 1, maxLength: 128, pattern: "\\S" })
  })
});
var ObservedContextEntryDataSchema = typebox_exports.Union([
  ReadResultDataSchema,
  ToolResultDataSchema,
  UserExplicitDataSchema,
  DomainEvidenceDataSchema
]);
var ObservedContextDataSchema = exactObject3({
  branchRef: typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
  entries: typebox_exports.Array(ObservedContextEntryDataSchema, { maxItems: 2e3 })
});
var InventoryNominationDataSchema = exactObject3({
  kind: typebox_exports.Literal("inventory-source"),
  inventorySourceId: IdentifierSchema,
  descriptorSha256: Sha256Schema,
  contentSha256: typebox_exports.Optional(Sha256Schema)
});
var ObservedNominationDataSchema = exactObject3({
  kind: typebox_exports.Literal("observed-context"),
  observedContextId: IdentifierSchema,
  descriptorSha256: Sha256Schema
});
var ContextNominationDataSchema = typebox_exports.Union([
  InventoryNominationDataSchema,
  ObservedNominationDataSchema
]);
var ContextSelectionProposalDataSchema = exactObject3({
  questionSha256: Sha256Schema,
  nominations: typebox_exports.Array(ContextNominationDataSchema, { maxItems: 256 }),
  selectionBasis: typebox_exports.Array(NonEmptyTextSchema, {
    minItems: 1,
    maxItems: 32,
    uniqueItems: true
  })
});
var inventoryBrand = Symbol("ContextInventory");
var observedBrand = Symbol("ObservedContext");
var proposalBrand = Symbol("ContextSelectionProposal");
var selectionBrand = Symbol("ContextSelection");
function schemaIssues2(schema, value) {
  return [...Errors2(schema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
}
function decodeContextInventoryData(value) {
  try {
    return Parse(ContextInventoryDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Context inventory has an invalid representation.",
      { issues: schemaIssues2(ContextInventoryDataSchema, value) },
      { cause: error }
    );
  }
}
function decodeObservedContextData(value) {
  try {
    return Parse(ObservedContextDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Observed context has an invalid representation.",
      { issues: schemaIssues2(ObservedContextDataSchema, value) },
      { cause: error }
    );
  }
}
function decodeContextSelectionProposalData(value) {
  try {
    return Parse(ContextSelectionProposalDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Context selection proposal has an invalid representation.",
      { issues: schemaIssues2(ContextSelectionProposalDataSchema, value) },
      { cause: error }
    );
  }
}
...
function uniqueText(values, path) {
  const normalized = values.map(
    (value, index) => text(value, `${path}/${index}`)
  );
  const seen = /* @__PURE__ */ new Set();
  for (const value of normalized) {
    if (seen.has(value))
...
function parseSource(data, index) {
...
function parseCapability(data, index) {
...
function parseContextInventory(data) {
  const sources = data.sources.map(parseSource).toSorted((left, right) => left.id.localeCompare(right.id));
  const capabilities2 = data.capabilities.map(parseCapability).toSorted((left, right) => left.id.localeCompare(right.id));
  const ids2 = /* @__PURE__ */ new Set();
  const physical = /* @__PURE__ */ new Set();
  for (const source of sources) {
    if (ids2.has(source.id))
      throw new JudgmentParseError(
        `Duplicate inventory source ID: ${source.id}.`
      );
    ids2.add(source.id);
    const location = source.kind === "pi-skill" ? source.provenance.path ?? source.id : source.path;
  ...
  const frozenCapabilities = Object.freeze(capabilities2);
  const inventory = {
    [inventoryBrand]: true,
    ...
    inventorySha256: sha256(
      canonicalJson({
        ...
        capabilities: frozenCapabilities.map(
          (capability) => capabilityIdentity(capability)
        )
      })
    )
  };
...
function observedIdentity(entry) {
  const base = {
    id: entry.id,
    kind: entry.kind,
  ...
  switch (entry.kind) {
    case "read-result":
      return {
        ...
        ...entry.inventorySourceId ? { inventorySourceId: entry.inventorySourceId } : {}
      };
    case "tool-result":
      return {
        ...base,
...
function parseObserved(data, index) {
  ...
  switch (data.kind) {
    case "read-result":
      entry = Object.freeze({
        ...
        ...data.inventorySourceId ? { inventorySourceId: data.inventorySourceId } : {}
      });
      break;
    case "tool-result":
      entry = Object.freeze({
...
function observedProvenanceKey(entry) {
  switch (entry.kind) {
    case "read-result":
    case "tool-result":
...
function parseObservedContext(data) {
  const entries = data.entries.map(parseObserved).toSorted(
    (left, right) => left.sequence - right.sequence || left.id.localeCompare(right.id)
  );
  const ids2 = /* @__PURE__ */ new Set();
  const provenanceKeys = /* @__PURE__ */ new Set();
...
function parseContextSelectionProposal(data) {
  const nominations = data.nominations.map((nomination) => Object.freeze({ ...nomination })).toSorted((left, right) => {
    const leftId = left.kind === "inventory-source" ? left.inventorySourceId : left.observedContextId;
    const rightId = right.kind === "inventory-source" ? right.inventorySourceId : right.observedContextId;
    return `${left.kind}:${leftId}`.localeCompare(`${right.kind}:${rightId}`);
  });
  const selected = /* @__PURE__ */ new Set();
  for (const nomination of nominations) {
    const id = nomination.kind === "inventory-source" ? `inventory:${nomination.inventorySourceId}` : `observed:${nomination.observedContextId}`;
    if (selected.has(id))
      throw new JudgmentParseError(
...
function bindingIdentity(binding) {
  return {
    ...
    ...binding.expectedContentSha256 ? { expectedContentSha256: binding.expectedContentSha256 } : {}
  };
}
function selectContext(input) {
  ...
  const selectionSha256 = sha256(
    canonicalJson({
      ...
      selectedObservedContext: frozenObserved.map(
        (entry) => observedIdentity(entry)
      ),
...
// src/coverage.ts
function exactObject4(properties) {
  return typebox_exports.Object(properties, { additionalProperties: false });
}
var MaterialIds = typebox_exports.Array(
  typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
  { minItems: 1, maxItems: 256, uniqueItems: true }
);
var OptionalMaterialIds = typebox_exports.Array(
  typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
  { maxItems: 256, uniqueItems: true }
);
var Evaluator = exactObject4({
  id: IdentifierSchema,
  version: typebox_exports.String({ minLength: 1, maxLength: 128, pattern: "\\S" })
});
var ContributionBase = {
  materialId: typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
  useAs: ContextUseSchema,
  contribution: NonEmptyTextSchema
};
var AgentContribution = exactObject4({
  ...ContributionBase,
  assurance: typebox_exports.Literal("agent-asserted")
});
var DomainContribution = exactObject4({
  ...ContributionBase,
  assurance: typebox_exports.Literal("domain-verified"),
  evaluator: Evaluator,
  evidenceMaterialIds: MaterialIds
});
var UserContribution = exactObject4({
  ...ContributionBase,
  assurance: typebox_exports.Literal("user-accepted"),
  userEventId: typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" })
});
var ContextContributionDataSchema = typebox_exports.Union([
  AgentContribution,
  DomainContribution,
  UserContribution
]);
var ContextConflictDataSchema = exactObject4({
  materialIds: typebox_exports.Array(
    typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
    { minItems: 2, maxItems: 256, uniqueItems: true }
  ),
  description: NonEmptyTextSchema
});
var ContextLimitationDataSchema = exactObject4({
  basisMaterialIds: OptionalMaterialIds,
  description: NonEmptyTextSchema
});
var ContextCoverageProposalDataSchema = exactObject4({
  status: typebox_exports.Union([
    typebox_exports.Literal("sufficient"),
    typebox_exports.Literal("needs-evidence")
  ]),
  contributions: typebox_exports.Array(ContextContributionDataSchema, { maxItems: 256 }),
  conflicts: typebox_exports.Array(ContextConflictDataSchema, { maxItems: 128 }),
  limitations: typebox_exports.Array(ContextLimitationDataSchema, { maxItems: 128 })
});
var proposalBrand2 = Symbol("ContextCoverageProposal");
var coverageBrand = Symbol("ContextCoverage");
function issues2(value) {
  return [...Errors2(ContextCoverageProposalDataSchema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
}
function decodeContextCoverageProposalData(value) {
  try {
    return Parse(ContextCoverageProposalDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Context coverage proposal has an invalid representation.",
      { issues: issues2(value) },
      { cause: error }
    );
  }
}
...
function ids(values, path) {
  const normalized = values.map(
    (value, index) => text2(value, `${path}/${index}`)
  );
  const seen = /* @__PURE__ */ new Set();
  for (const value of normalized) {
    if (seen.has(value))
...
function contributionBody(data, index) {
  const base = {
    materialId: text2(data.materialId, `/contributions/${index}/materialId`),
    useAs: data.useAs,
    contribution: text2(
      data.contribution,
  ...
  switch (data.assurance) {
    ...
    default:
      return assertNever2(data);
  }
}
function contributionIdentity(value) {
  const base = {
  ...
  switch (value.assurance) {
    ...
    default:
      return assertNever2(value);
  }
}
function parseContribution(data, index) {
  ...
  switch (body.assurance) {
    ...
    case "user-accepted":
      return Object.freeze({ ...body, contributionId });
    default:
      return assertNever2(body);
  }
}
function parseConflict(data, index) {
  const body = {
    materialIds: ids(data.materialIds, `/conflicts/${index}/materialIds`),
    description: text2(data.description, `/conflicts/${index}/description`)
  };
  return Object.freeze({
    ...body,
...
function parseLimitation(data, index) {
  const body = {
    basisMaterialIds: ids(
      data.basisMaterialIds,
      `/limitations/${index}/basisMaterialIds`
    ),
    description: text2(data.description, `/limitations/${index}/description`)
  };
  return Object.freeze({
    ...body,
...
function parseContextCoverageProposal(data) {
  const contributions = Object.freeze(
    data.contributions.map(parseContribution).toSorted(
      (left, right) => left.contributionId.localeCompare(right.contributionId)
    )
  );
  ...
  const conflicts = Object.freeze(
    data.conflicts.map(parseConflict).toSorted(
  ...
  const limitations = Object.freeze(
    data.limitations.map(parseLimitation).toSorted(
      (left, right) => left.limitationId.localeCompare(right.limitationId)
    )
  );
  const limitationIds = /* @__PURE__ */ new Set();
...
function verifyAssurance(value, sealed) {
  if (value.assurance === "agent-asserted") return;
  if (value.assurance === "domain-verified") {
    const evidence = value.evidenceMaterialIds.map(
      (id) => material(sealed, id)
    );
    const matches2 = evidence.some(
      (member) => member.origin.kind === "observed-context" && member.origin.entry.kind === "domain-evidence" && member.origin.entry.evaluator.id === value.evaluator.id && member.origin.entry.evaluator.version === value.evaluator.version
    );
    if (!matches2)
      throw new JudgmentParseError(
  ...
  const matches = sealed.members.some(
    (member) => member.origin.kind === "observed-context" && member.origin.entry.kind === "user-explicit" && member.origin.entry.userEventId === value.userEventId
  );
  if (!matches)
    throw new JudgmentParseError(
...
function assessContextCoverage(input) {
  ...
  if (proposal.status === "needs-evidence" && proposal.conflicts.length === 0 && proposal.limitations.length === 0)
    throw new JudgmentParseError(
...
function judgmentPolicyDirections(policy) {
  const references = policy.references.flatMap((reference2) => [
    `### \`${reference2.path}\``,
    "",
    list(reference2.when),
    ""
  ]);
  const markdown = [
    "## Context Directions",
...
// src/lifecycle.ts
var CONTEXT_JUDGMENT_EVENT_PROTOCOL = "judgment-event/v1";
function exactObject5(properties) {
  return typebox_exports.Object(properties, { additionalProperties: false });
}
var Basis = typebox_exports.Array(NonEmptyTextSchema, {
  minItems: 1,
  maxItems: 32,
  uniqueItems: true
});
var ContextApplicabilityDataSchema = typebox_exports.Union([
  exactObject5({ kind: typebox_exports.Literal("applicable"), basis: Basis }),
  exactObject5({
    kind: typebox_exports.Literal("not-applicable"),
    reason: NonEmptyTextSchema,
    evidence: Basis
  }),
  exactObject5({ kind: typebox_exports.Literal("needs-context"), missingContext: Basis })
]);
function issues3(value) {
  return [...Errors2(ContextApplicabilityDataSchema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
}
function decodeContextApplicabilityData(value) {
  try {
    return Parse(ContextApplicabilityDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Context applicability has an invalid representation.",
      { issues: issues3(value) },
      { cause: error }
    );
  }
}
...
function unique(values, path) {
  const normalized = values.map(
    (value, index) => text3(value, `${path}/${index}`)
  );
  const seen = /* @__PURE__ */ new Set();
  for (const value of normalized) {
    if (seen.has(value))
...
function parseContextApplicability(data) {
  switch (data.kind) {
    case "applicable":
      return Object.freeze({
        kind: data.kind,
    ...
    case "not-applicable":
      return Object.freeze({
        kind: data.kind,
        reason: text3(data.reason, "/reason"),
    ...
    default:
      return assertNever3(data);
  }
}
function startJudgment(question) {
  return Object.freeze({ question, status: "started", revision: 0 });
}
function applicabilityRecorded(input) {
  return Object.freeze({
...
function selectionRecorded(input) {
  return Object.freeze({
    protocol: CONTEXT_JUDGMENT_EVENT_PROTOCOL,
    judgmentId: input.state.question.judgmentId,
...
function sealedContextRecorded(input) {
  return Object.freeze({
    protocol: CONTEXT_JUDGMENT_EVENT_PROTOCOL,
    judgmentId: input.state.question.judgmentId,
...
function coverageRecorded(input) {
  return Object.freeze({
    protocol: CONTEXT_JUDGMENT_EVENT_PROTOCOL,
    judgmentId: input.state.question.judgmentId,
...
function outcomeRecorded(input) {
  return Object.freeze({
    protocol: CONTEXT_JUDGMENT_EVENT_PROTOCOL,
    judgmentId: input.state.question.judgmentId,
...
function applicabilityIdentity(value) {
  switch (value.kind) {
    ...
    default:
      return assertNever3(value);
  }
}
function sameEvent(state, event) {
  switch (event.kind) {
    case "applicability-recorded":
      return Boolean(
        state.applicability && canonicalJson(applicabilityIdentity(state.applicability)) === canonicalJson(applicabilityIdentity(event.applicability))
      );
    case "selection-recorded":
      return state.revision === event.revision && state.selection?.selectionSha256 === event.selection.selectionSha256;
    case "sealed-context-recorded":
      return state.revision === event.revision && state.sealedContext?.sealedContextSha256 === event.sealedContext.sealedContextSha256;
...
function assertEvent(state, event) {
...
function transitionJudgment(state, event) {
  ...
  switch (event.kind) {
    case "applicability-recorded": {
      ...
      let status = "terminal-needs-context";
    ...
    case "selection-recorded": {
      if (!["selection-open", "selected", "sealed", "covered"].includes(
        state.status
      ))
        throw new JudgmentTransitionError(
      ...
      if (event.selection.judgmentId !== state.question.judgmentId || event.selection.questionSha256 !== state.question.questionSha256)
        throw new JudgmentTransitionError(
    ...
    case "sealed-context-recorded": {
      ...
      if (event.revision !== state.revision || event.sealedContext.selectionSha256 !== state.selection.selectionSha256)
        throw new JudgmentTransitionError(
    ...
    case "coverage-recorded": {
      ...
      if (event.coverage.sealedContextSha256 !== state.sealedContext.sealedContextSha256)
        throw new JudgmentTransitionError(
    ...
    case "outcome-recorded": {
      ...
      if (event.outcome.sealedContextSha256 !== state.sealedContext.sealedContextSha256 || event.outcome.coverageSha256 !== state.coverage.coverageSha256)
        throw new JudgmentTransitionError(
...
function replayJudgmentEvents(initial, events) {
...
// src/events.ts
var JUDGMENT_EVENT_PROTOCOL = "judgment-event/v1";

// src/outcome.ts
function exactObject6(properties) {
  return typebox_exports.Object(properties, { additionalProperties: false });
}
var Citation = exactObject6({
  contributionId: typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
  locator: typebox_exports.Optional(
    typebox_exports.String({ minLength: 1, maxLength: 1e3, pattern: "\\S" })
  ),
...
var Contextual = exactObject6({
  ...Base2,
  kind: typebox_exports.Literal("contextual-judgment"),
  citedUses: typebox_exports.Array(Citation, { maxItems: 256 }),
  rationale: NonEmptyTextSchema,
  artifact: NonEmptyTextSchema,
  stopEvidence: typebox_exports.Array(NonEmptyTextSchema, {
    minItems: 1,
    maxItems: 32,
    uniqueItems: true
  })
});
var NeedsEvidence = exactObject6({
  ...Base2,
  kind: typebox_exports.Literal("needs-evidence"),
  unresolvedIds: typebox_exports.Array(
    typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
    { minItems: 1, maxItems: 256, uniqueItems: true }
  ),
  evidenceNeeded: typebox_exports.Array(NonEmptyTextSchema, {
    minItems: 1,
    maxItems: 32,
    uniqueItems: true
  }),
  resolutionOwner: typebox_exports.Union([
    typebox_exports.Literal("agent"),
    typebox_exports.Literal("user"),
    typebox_exports.Literal("environment"),
    typebox_exports.Literal("unknown")
  ]),
  artifact: typebox_exports.Optional(NonEmptyTextSchema)
});
var Emergent = exactObject6({
  ...Base2,
  kind: typebox_exports.Literal("emergent-question"),
...
var JudgmentProposalDataSchema = typebox_exports.Union([
  Contextual,
  NeedsEvidence,
  Emergent
]);
var proposalBrand3 = Symbol("JudgmentProposal");
function issues4(value) {
  return [...Errors2(JudgmentProposalDataSchema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
}
function decodeJudgmentProposalData(value) {
  try {
    return Parse(JudgmentProposalDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Judgment proposal has an invalid representation.",
      { issues: issues4(value) },
      { cause: error }
    );
  }
}
function text4(value, path) {
  if (value !== value.trim() || !value.trim())
    throw new JudgmentParseError(
      `Expected directly normalized non-blank text at ${path}.`
    );
...
function unique2(values, path) {
  const normalized = values.map(
    (value, index) => text4(value, `${path}/${index}`)
  );
  const seen = /* @__PURE__ */ new Set();
  for (const value of normalized) {
    if (seen.has(value))
...
function parseJudgmentProposal(data) {
  const base = {
    [proposalBrand3]: true,
  ...
  switch (data.kind) {
    case "contextual-judgment": {
      const citedUses = Object.freeze(
        data.citedUses.map(
          (citation, index) => Object.freeze({
            contributionId: text4(
      ...
      for (const citation of citedUses) {
        if (ids2.has(citation.contributionId))
          throw new JudgmentParseError(
            `Duplicate contribution citation: ${citation.contributionId}.`
          );
        ids2.add(citation.contributionId);
      }
      return Object.freeze({
        ...base,
        kind: data.kind,
        citedUses,
        rationale: text4(data.rationale, "/rationale"),
        artifact: text4(data.artifact, "/artifact"),
        stopEvidence: unique2(data.stopEvidence, "/stopEvidence")
      });
    }
    case "needs-evidence":
      return Object.freeze({
        ...base,
        kind: data.kind,
        unresolvedIds: unique2(data.unresolvedIds, "/unresolvedIds"),
        evidenceNeeded: unique2(data.evidenceNeeded, "/evidenceNeeded"),
        resolutionOwner: data.resolutionOwner,
        ...data.artifact ? { artifact: text4(data.artifact, "/artifact") } : {}
      });
    case "emergent-question":
      return Object.freeze({
        ...base,
        kind: data.kind,
        question: text4(data.question, "/question"),
        reason: text4(data.reason, "/reason"),
        artifact: text4(data.artifact, "/artifact"),
        stopEvidence: unique2(data.stopEvidence, "/stopEvidence")
      });
    default:
      return assertNever4(data);
  }
}
function common(input) {
  const { question, selectedContext, coverage, proposal } = input;
  if (question.judgmentId !== selectedContext.judgmentId || question.questionSha256 !== selectedContext.questionSha256)
    throw new JudgmentParseError(
  ...
  if (coverage.sealedContextSha256 !== selectedContext.sealedContextSha256 || coverage.questionSha256 !== question.questionSha256)
    throw new JudgmentParseError(
      "Coverage and sealed context do not describe one judgment."
    );
}
function unresolved(coverage) {
  return Object.freeze(
    [
...
function identity(question, proposal) {
  const base = {
    kind: proposal.kind,
    judgmentId: question.judgmentId,
    questionSha256: question.questionSha256,
  ...
  switch (proposal.kind) {
    ...
    default:
      return assertNever4(proposal);
  }
}
function concludeJudgment(input) {
  common(input);
  ...
  switch (proposal.kind) {
    ...
    case "emergent-question": {
      if (question.question.toLocaleLowerCase().replace(/\s+/gu, " ") === proposal.question.toLocaleLowerCase().replace(/\s+/gu, " "))
        throw new JudgmentParseError(
...
// src/question.ts
function exactObject7(properties) {
  return typebox_exports.Object(properties, { additionalProperties: false });
}
var DynamicJudgmentQuestionDataSchema = exactObject7({
  judgmentId: IdentifierSchema,
  owner: PolicyOwnerDataSchema,
  policySha256: typebox_exports.Optional(Sha256Schema),
  question: typebox_exports.String({ minLength: 1, maxLength: 4e3, pattern: "\\S" }),
  basisMaterialIds: typebox_exports.Array(
    typebox_exports.String({ minLength: 1, maxLength: 300, pattern: "\\S" }),
    {
      maxItems: 128,
      uniqueItems: true
...
var dynamicQuestionBrand = Symbol("DynamicJudgmentQuestion");
function issues5(value) {
  return [...Errors2(DynamicJudgmentQuestionDataSchema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
...
function sortedIds(values) {
  const normalized = values.map(
    (value, index) => normalizedText(value, `/basisMaterialIds/${index}`)
  );
  const seen = /* @__PURE__ */ new Set();
  for (const value of normalized) {
    if (seen.has(value)) {
...
function decodeDynamicJudgmentQuestionData(value) {
  try {
    return Parse(DynamicJudgmentQuestionDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Dynamic judgment question has an invalid representation.",
      { issues: issues5(value) },
      { cause: error }
    );
  }
}
function parseDynamicJudgmentQuestion(data) {
  ...
  const dynamicQuestion = {
    ...
    basisMaterialIds,
    branchRef,
    questionSha256: sha256(canonicalJson(identity2))
  };
  return Object.freeze(dynamicQuestion);
}

// src/sealed-context.ts
function exactObject8(properties) {
  return typebox_exports.Object(properties, { additionalProperties: false });
}
var ContextContentPartDataSchema = typebox_exports.Union([
  exactObject8({
    kind: typebox_exports.Literal("text"),
    text: typebox_exports.String({ maxLength: 48e3 })
  }),
  exactObject8({
    kind: typebox_exports.Literal("image"),
    mediaType: typebox_exports.String({ minLength: 1, maxLength: 128 }),
    contentSha256: Sha256Schema
  })
]);
var SealedContextMemberDataSchema = exactObject8({
  bindingId: typebox_exports.String({ minLength: 1, maxLength: 300 }),
  contentSha256: Sha256Schema,
  isError: typebox_exports.Boolean(),
  truncated: typebox_exports.Boolean(),
  parts: typebox_exports.Array(ContextContentPartDataSchema, {
    minItems: 1,
    maxItems: 64
  })
});
var SealedContextProposalDataSchema = exactObject8({
  selectionSha256: Sha256Schema,
  members: typebox_exports.Array(SealedContextMemberDataSchema, { maxItems: 256 })
});
var sealedBrand = Symbol("SealedContext");
function issues6(value) {
  return [...Errors2(SealedContextProposalDataSchema, value)].map((issue) => ({
    path: issue.instancePath || "/",
    message: issue.message,
    keyword: issue.keyword
  }));
}
function decodeSealedContextProposalData(value) {
  try {
    return Parse(SealedContextProposalDataSchema, value);
  } catch (error) {
    throw new JudgmentParseError(
      "Sealed context proposal has an invalid representation.",
      { issues: issues6(value) },
      { cause: error }
    );
  }
}
function part(data, path) {
  if (data.kind === "text")
...
function partIdentity(value) {
  return value.kind === "text" ? { kind: "text", text: value.text } : {
    kind: "image",
    mediaType: value.mediaType,
    contentSha256: value.contentSha256
  };
}
function contextContentSha256(parts) {
  return sha256(canonicalJson(parts.map(partIdentity)));
}
function origin(selection, binding) {
...
function parseSealedContext(data, selection) {
  ...
  for (const binding of selection.bindings) {
    ...
    const parts = Object.freeze(
      dataMember.parts.map(
        (value, index) => part(value, `/members/${binding.bindingId}/parts/${index}`)
      )
    );
    ...
    if (binding.expectedContentSha256 && binding.expectedContentSha256 !== actual)
      throw new JudgmentParseError(
    ...
    } else if (dataMember.isError !== selectedOrigin.entry.isError || dataMember.truncated !== selectedOrigin.entry.truncated) {
      throw new JudgmentParseError(
  ...
  const frozenMembers = Object.freeze(
    members.toSorted(
      (left, right) => left.materialId.localeCompare(right.materialId)
    )
  );
...
async function readUtf8(path, maxBytes) {
...
async function readJudgmentPolicyFile(input) {
  ...
  try {
    ...
    for (const reference2 of authoring.references) {
      const lexicalReference = resolve(lexicalRoot, reference2.path);
      if (!contained(lexicalRoot, lexicalReference))
        throw new JudgmentParseError(
          `Prepared reference escapes its policy root: ${reference2.path}.`
        );
      const realReference = await realpath(lexicalReference);
      if (!contained(realRoot, realReference) || !contained(realAllowedRoot, realReference))
        throw new JudgmentParseError(
          `Prepared reference symlink escapes its policy root: ${reference2.path}.`
        );
      const metadata = await stat(realReference);
      if (!metadata.isFile())
        throw new JudgmentParseError(
          `Prepared reference is not a regular file: ${reference2.path}.`
        );
    }
    return Object.freeze({
      path: lexicalPath,
...
async function loadOptionalJudgmentPolicyFile(input) {
  try {
    return Object.freeze({
      kind: "loaded",
    ...
    if (missingFile(error) || error instanceof JudgmentParseError && missingFile(error.cause))
...
var MAX_SEALED_MEMBER_BYTES = 48e3;
var MAX_SEALED_CONTEXT_BYTES = 256e3;
function isContained(root, candidate) {
  const path = relative2(root, candidate);
  return path === "" || !path.startsWith("..") && !path.startsWith("/");
}
function createNodeLocalReferenceReader(policyRoot) {
  const lexicalRoot = resolve2(policyRoot);
  let resolvedRoot;
  const root = () => resolvedRoot ??= realpath2(lexicalRoot);
  return {
    async read(source, options) {
      const lexicalTarget = resolve2(lexicalRoot, source.path);
      ...
      try {
        ...
        const metadata = await stat2(realTarget);
        if (!metadata.isFile()) {
          throw new ContextSealError(
...
async function sealContext(selection, acquisition, options = {}) {
...
function provenance2(sourceInfo) {
  return {
...
function capabilities(tools, activeToolNames) {
  const active = new Set(activeToolNames);
  return tools.map((tool) => ({
    id: descriptorId("tool", {
      name: tool.name,
      provenance: provenance2(tool.sourceInfo)
    }),
    kind: "pi-tool",
    name: tool.name,
...
function buildPiContextInventory(input) {
...
function toolCalls(entries) {
  ...
  for (const entry of entries) {
    ...
    for (const raw of message.content) {
      const part2 = objectValue(raw);
      if (part2?.type !== "toolCall" || typeof part2.id !== "string" || typeof part2.name !== "string")
        continue;
      if (calls.has(part2.id))
        throw new JudgmentParseError(
          `Duplicate tool call ID on the active branch: ${part2.id}.`
        );
      calls.set(part2.id, {
        id: part2.id,
        name: part2.name,
        argumentsSha256: sha256(
          canonicalJson(jsonValueFromUnknown(part2.arguments ?? {}))
        )
      });
    }
  }
...
function contentParts(value) {
  ...
  for (const [index, raw] of value.entries()) {
    const part2 = objectValue(raw);
    if (part2?.type === "text" && typeof part2.text === "string") {
      parts.push({ kind: "text", text: part2.text });
      continue;
    }
    if (part2?.type === "image" && typeof part2.data === "string" && typeof part2.mimeType === "string") {
      parts.push({
        kind: "image",
        mediaType: part2.mimeType,
        contentSha256: sha256(Buffer.from(part2.data, "base64"))
      });
      continue;
    }
    throw new JudgmentParseError(
...
function activeBranchToolResultIdentities(entries) {
...
function userDecisions(entries) {
  const decisions = /* @__PURE__ */ new Map();
  for (const [sequence, entry] of entries.entries()) {
    if (entry.type !== "message" || !entry.id) continue;
    const message = objectValue(entry.message);
    if (message?.role !== "user") continue;
    const parts = typeof message.content === "string" ? Object.freeze([{ kind: "text", text: message.content }]) : contentParts(message.content);
...
function resolveObservedContext(input) {
  ...
  for (const nomination of input.toolNominations) {
    ...
    entries.push({
      ...
      ...result.toolName === "read" && nomination.inventorySourceId ? { inventorySourceId: nomination.inventorySourceId } : {}
    });
  }
  ...
  return Object.freeze({
    observedContext,
    async acquireObservedContext(entry) {
...
function resolveObservedToolContext(input) {
...
export {
  ...
  CapabilityDescriptorDataSchema,
  CompiledJudgmentPolicyDataSchema,
  ContextApplicabilityDataSchema,
  ContextAttempt,
  ContextConflictDataSchema,
  ContextContentPartDataSchema,
  ContextContributionDataSchema,
  ContextCoverageProposalDataSchema,
  ContextInventoryDataSchema,
  ContextLimitationDataSchema,
  ContextNominationDataSchema,
  ContextSealError,
  ContextSelectionProposalDataSchema,
  ContextSourceDescriptorDataSchema,
  ContextUseSchema,
  DynamicJudgmentQuestionDataSchema,
  IdentifierSchema,
  JUDGMENT_EVENT_PROTOCOL,
  JudgmentAuthoringJsonSchema,
  JudgmentAuthoringPolicyDataSchema,
  JudgmentError,
  JudgmentParseError,
  JudgmentProposalDataSchema,
  JudgmentTransitionError,
  MAX_JUDGMENT_POLICY_BYTES,
  MAX_SEALED_CONTEXT_BYTES,
  MAX_SEALED_MEMBER_BYTES,
  NonEmptyTextSchema,
  ObservedContextDataSchema,
  ObservedContextEntryDataSchema,
  PolicyOwnerDataSchema,
  PreparedReferencePolicyDataSchema,
  SealedContextMemberDataSchema,
  SealedContextProposalDataSchema,
  Sha256Schema,
  activeBranchToolResultIdentities,
  applicabilityRecorded,
  assessContextCoverage,
  buildPiContextInventory,
  canonicalCompiledJudgmentPolicy,
  canonicalJson,
  canonicalJudgmentAuthoringPolicy,
  ...
  decodeJudgmentAuthoringPolicyData,
  ...
  parsePolicyOwner,
  parsePreparedReferencePath,
  parseSealedContext,
  policyOwnerIdentity,
  readJudgmentPolicyFile,
  replayJudgmentEvents,
